...
def main(request):
  ...
  if bucket is not None:
    _logger.info('Using {bucket} for storing tweets.'.format(bucket=bucket))
  userBucket = messageJSON.get('userBucket', None)
  if userBucket == '': userBucket = None
  if userBucket is not None:
    _logger.info('Using {bucket} for storing users.'.format(bucket=userBucket))
  ...
  for term in query:
    _logger.debug('Waiting on rate limit to replenish.')
    ...
    _logger.debug('Done querying for {query}.'.format(query=term))
